...
export default function LinkedInOutput({ content, author, style }: LinkedInOutputProps) {
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col items-center gap-3 overflow-y-auto bg-background py-3">
      <div className="mx-auto w-[400px] scale-90"> {/* Reduced width and applied scale */}
        <div className="font-system overflow-hidden rounded-lg bg-white shadow ring-1 ring-inset ring-gray-200">
          <div className="py-3 pl-3 pr-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="relative inline-block shrink-0">
                    ...
                    />
                    <span className="absolute bottom-0 right-0 inline-flex size-3 items-center justify-center rounded-full bg-[#1052B8] text-white ring-2 ring-white">
                      <Image
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-semibold text-gray-900">{author}</p>
                    <p className="truncate text-xs font-normal text-gray-400">AI-Generated Content</p>
                    <div className="flex items-center gap-1">
                      <span className="text-xs font-normal text-gray-400">Now</span>
            ...
            <div className="relative mt-3 whitespace-pre-wrap text-black text-sm">
              {content}
            </div>  {/* Changed text color to black */}
          </div>
...
function ActionButton({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex items-center justify-center gap-1.5 rounded-lg px-1 py-1 text-xs font-semibold text-gray-500 hover:bg-gray-100">
      {icon}
      {text}
    </div>
  );
...
// Icons remain the same, just adjusting their size
function LikeIcon() {
  return (
    <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" viewBox="0 0 26 26" className="size-4">
...
function SendIcon() {
  return (
    <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" viewBox="0 0 26 26" className="size-4">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21.354 3.644 2.43 8.98"></path>
    </svg>
  );
}
